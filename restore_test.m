function[ lb_ang,k ]=restore_test(gvf,BW2,L1,s1,s2)
...
if((thisBB(4)<2*thisBB(3) || thisBB(3)>3*thisBB(4) ) && thisBB(3)*thisBB(4)>=10 && thisBB(3)*thisBB(4)<=0.3 * size(gvf,1)* size(gvf,2) && thisBB(3)<=0.3*s1 && thisBB(4)<=0.3*s2)
...
rc3=[0 0];
          ...
          for k3=1:size(rc3,1)
              xval3=rc3(k3,1);
              yval3=rc3(k3,2);
if( (flags==1 || flags1==1 ) && in_angle <90  )
                lb_ang(xval3,yval3)=1;
elseif((flags==1 & flags1==1 ) && in_angle <90)
lb_ang(xval3,yval3)=1;
else
lb_ang(xval3,yval3)=0;
